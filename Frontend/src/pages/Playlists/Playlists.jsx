  ...
  return (
    <div className="max-w-[1280px] mx-auto p-4 md:p-6 lg:px-8">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        <motion.h1 className="flex items-center gap-3 text-2xl md:text-[28px] font-light text-white/90 m-0" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          <HiOutlineFolderOpen className="text-teal-primary" /> Your <span className="text-teal-gradient">Playlists</span>
        </motion.h1>
        ...
        {showCreate && (
          <motion.div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div className="bg-bg-elevated w-full max-w-[500px] border border-white/10 p-6 md:p-8 rounded-2xl shadow-2xl relative overflow-hidden" onClick={e => e.stopPropagation()}
              ...
              <h2 className="text-xl font-light text-white/90 m-0 mb-6 border-b border-white/10 pb-4">Create Playlist</h2>
              <form onSubmit={handleCreate} className="flex flex-col gap-5">
                <div className="flex flex-col gap-1.5 [&>label]:text-[13px] [&>label]:font-medium [&>label]:text-white/70 [&>label]:ml-1">
                  <label>Name</label>
                  <input className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-[14.5px] text-white/90 placeholder:text-white/40 focus:outline-none focus:border-teal-primary transition-colors" placeholder="Playlist name" value={newPlaylist.name}
                    onChange={e => setNewPlaylist({...newPlaylist, name: e.target.value})} />
                </div>
                <div className="flex flex-col gap-1.5 [&>label]:text-[13px] [&>label]:font-medium [&>label]:text-white/70 [&>label]:ml-1">
                  <label>Description</label>
                  <textarea className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-[14.5px] text-white/90 placeholder:text-white/40 focus:outline-none focus:border-teal-primary transition-colors resize-y min-h-[80px]" placeholder="Describe your playlist" rows={3}
          ...
          <motion.div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div className="bg-bg-elevated w-full max-w-[500px] border border-white/10 p-6 md:p-8 rounded-2xl shadow-2xl relative overflow-hidden" onClick={e => e.stopPropagation()}
              ...
              <div className="flex items-center justify-between mb-6 border-b border-white/10 pb-4">
                <h2 className="text-xl font-light text-white/90 m-0">Add Video to "{addVideoModal.playlistName}"</h2>
                <button className="flex items-center justify-center p-2 rounded-lg bg-bg-surface border border-white/10 text-white/60 hover:text-white hover:bg-white/5 transition-colors cursor-pointer outline-none"
                  onClick={() => { setAddVideoModal(null); setVideoUrl(''); }}>
              ...
              <form onSubmit={handleAddVideo} className="flex flex-col gap-5">
                <div className="flex flex-col gap-1.5 [&>label]:text-[13px] [&>label]:font-medium [&>label]:text-white/70 [&>label]:ml-1">
                  ...
                  <input
                    ...
                    autoFocus
                  />
                  <p className="text-[12px] text-white/50 m-0 ml-1">
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="w-full h-[280px] bg-white/5 rounded-2xl animate-pulse" />
          ))}
        </div>
      ) : playlists.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <motion.div key={pl._id} className="flex flex-col bg-bg-surface border border-white/10 rounded-2xl overflow-hidden group"
              ...
              <Link to={`/playlist/${pl._id}`} className="flex flex-col no-underline">
                <div className="relative w-full aspect-video bg-white/5 flex items-center justify-center overflow-hidden">
                  <HiOutlineFolderOpen className="text-4xl text-white/20" />
                ...
                <div className="flex flex-col p-5 gap-2">
                  <h3 className="text-lg font-light text-white/90 truncate m-0 group-hover:text-teal-soft transition-colors">{pl.name}</h3>
                  <p className="text-[13.5px] text-white/50 line-clamp-2 m-0 min-h-[40px] leading-relaxed">{pl.description}</p>
                  <span className="text-[12px] font-medium text-white/40 flex items-center gap-1.5 mt-1">
              ...
              <div className="px-5 pb-5 pt-0 mt-auto">
                <button
                  className="w-full !px-4 !py-2 bg-white/5 hover:bg-teal-primary/10 hover:text-teal-soft text-white/70 border border-white/5 hover:border-teal-primary/30 flex items-center justify-center gap-2 text-[13px] font-medium rounded-xl cursor-pointer transition-all outline-none"
        ...
        <div className="flex flex-col items-center justify-center py-20 px-5 text-center">
          ...
          <p className="text-sm text-white/50">Create your first playlist to organize videos</p>
        </div>
      )}
    </div>
  );
